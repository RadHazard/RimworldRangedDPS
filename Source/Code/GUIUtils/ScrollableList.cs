...
namespace RangedDPS.GUIUtils
{
    ...
    public abstract class ScrollableList<T>
    {
        ...
        public void Draw(in Rect inRect)
        {
            ...
            int firstVisibleEntry = Mathf.Clamp(Mathf.FloorToInt(scrollPosition.y / entryHeight), 0, entries.Count);
            int lastVisibleEntry = Mathf.Clamp(Mathf.CeilToInt((scrollPosition.y + inRect.height) / entryHeight) + 1, 0, entries.Count);
            ...
            try
            {
                ...
                {
                    T entry = entries[i];
                    DrawEntry(entry, new Rect(0, i * entryHeight, entryWidth, entryHeight));
                }
            }
            finally
            {
        ...
        /// </summary>
        /// <param name="inRect">In rect.</param>
        public abstract void DrawEntry(T entry, in Rect inRect);
    }
}
